      ...
      class Poller
        ...
        def run
          ...
          @options = DEFAULT_OPTS
                     .merge(Aws::Rails::SqsConsumer.config.to_h)
                     .merge(@options.to_h)
          validate_config
        ...
        def poll
          queue_url = Aws::Rails::SqsConsumer.config.queue_url_for(@options[:queue])
          @logger.info "Polling on: #{@options[:queue]} => #{queue_url}"
          client = Aws::Rails::SqsConsumer.config.client
        ...
        def parse_args(argv)
          ...
          parser.banner = "sqs_consumer [options]"
