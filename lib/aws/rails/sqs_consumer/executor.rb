  ...
  module Rails
    module SqsConsumer
      ...
      class Executor
        ...
        def execute(queue, message)
          @executor.post(message) do |message|
            begin
              @job_data = begin
                            { 'arguments' => [Aws::Json.load(message.data.body)] }
                          rescue
              ...
              @job_data['job_class'] = Aws::Rails::SqsConsumer.config.queues[queue.to_sym][:job_class_name] #'RemoteStore::Amazon::SqsPullJob'
              @job_data['job_id'] = message.data.message_id
              @job_data['retries_count'] = 0
              @job_data['queue_name'] = queue
              ...
              job_msg = @job_data ? "#{@job_data['job_id']}[#{@job_data['job_class']}]" : 'unknown job'
