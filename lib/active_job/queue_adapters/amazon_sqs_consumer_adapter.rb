...
module ActiveJob
  module QueueAdapters
    ...
    class AmazonSqsConsumerAdapter
      ...
      def _enqueue(job, body = nil, send_message_opts = {})
        body ||= job.serialize
        queue_url = Aws::Rails::SqsConsumer.config.queue_url_for(job.queue_name)
        ...
        if Aws::Rails::SqsConsumer.fifo?(queue_url)
          ...
          message_group_id ||= Aws::Rails::SqsConsumer.config.message_group_id

          send_message_opts[:message_group_id] = message_group_id
        end

        Aws::Rails::SqsConsumer.config.client.send_message(send_message_opts)
      end
